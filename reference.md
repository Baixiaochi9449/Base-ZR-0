...
## Optical Flow Head V2

- Date: 2026-09-09.
- Purpose: add optional `wan_vae_latent_v2` supervision while preserving `none` and `dense_regression_v1`.
- Files: `utils/optical_flow_config.py`, `utils/optical_flow_v2.py`, `utils/optical_flow_reader.py`, `utils/aux_data_contract.py`, `utils/stage06_dataset.py`, `utils/load_training_dataset.py`, `utils/optical_flow_loss.py`, `utils/optical_flow_checkpoint.py`, `model/optical_flow_aux_head.py`, `model/reasoning_vla_model.py`, `train_vla.py`, `scripts/calibrate_flow_color_scale.py`, `scripts/build_flow_latent_cache.py`, the Flow/Stage fixtures and V2 audit/training documents.
- Switch: existing `optical_flow_aux_type`; V2 additionally requires explicit `flow_vae_model_path` and locked `flow_color_scale`. Default remains `none`; Query-off/V1/none do not additionally import the V2 target module, parse the Wan path, instantiate the Wan VAE or access V2 cache. The existing Action Expert may independently depend on Diffusers, and V1 retains its original HDF5 label reads.
- Implementation source: adaptation of this repository's existing Optical Flow registry/loss/checkpoint contracts and RynnWorld-4D commit `2c748539849ada70d9148327e2195543ba3a921a`, specifically `utils/pre-process.py::PreProcess.load_components/encode_video` (`AutoencoderKLWan`, `latent_dist.mode()`, mean/std normalization). The Adapter is a custom addition because the reference repository has no Difference-Query Adapter. Flow semantics were checked against FD-ID-FlowVLA commit `3824d36cdf76bf0a9d537635de92a38f3920e9a3`, `Flow-image-generation/stage06_flow/{generate.py,flow_ops.py,storage.py}` and the vendored MegaFlow `megaflow/utils/flow_viz.py`.
- Data contract: MolmoAct Tabletop Stage06 H5 stores float16 `[T,2,224,224]` source-to-target flow in normalized source-image extent and uint8 masks. The generator divides pixel `u/v` by source `(W-1)/(H-1)`, then bilinear-resizes vectors to the 224 grid without changing normalized magnitude; masks use nearest-neighbor resize and mark finite forward destinations within the source bounds. V2 supervision uses the shared selector in `utils/optical_flow_v2.py`: per-source nominal delta (or validated `flow_delta_frames` fallback), explicit `flow_label_source` (default 1), and whole-sample valid fraction (default 0.95). A read-only scalar-field scan of manifest SHA256 `0438bbe865b2df8da7515b9bfa995483ef0ceea8161756898da57ef6a8c6023c` found 1881 episodes/310743 frames, 273123 delta20 rows, 35739 positive short-delta tails, 1881 source-2 identity rows, and 265136 rows satisfying delta20 + source1 + valid fraction >=0.95 (85.3232% of all frames; 97.0705% of delta20 rows). Tail short-delta and nonmatching label sources are excluded before both the loss and its distributed denominator are computed.
- Target: fixed Baker/Middlebury color wheel with verified source-to-target `u` right-positive, `v` down-positive, RGB order and `atan2(-v,-u)` orientation. It emits unquantized float RGB in [0,1], white zero-flow/invalid pixels, and uses one locked global scale from deterministic training-split calibration without per-image/batch normalization. VAE input is one frame `[B,3,1,224,224]`, normalized to [-1,1]; latent uses deterministic posterior mode and `(latent-mean)/std`; `Tz=1` is mandatory.
- Adapter: final-norm trailing Difference Queries only; LayerNorm/Linear memory, learned spatial tokens, two Pre-LN cross-attention + MLP residual blocks, and dynamic latent reshape. Action Expert input is unchanged.
- Loss: latent-only FP32 per-sample element-mean MSE. VAE is eval/frozen/no-grad; Query and Adapter remain differentiable. Decoder is not used in training.
- VAE lifecycle: `WanFlowTargetBuilder` owns one lazily selected online encoder device, moves the same instance only when the prediction device changes, and keeps it frozen/eval. Normal training passes the Accelerator rank device at construction. Deterministic target encoding is under `no_grad`; target tensors return as FP32 on the prediction device, while Query-to-Adapter remains differentiable. Strict cache mode creates no VAE instance.
- Cache: online generation and strict reading share versioned target protocol bytes independent of online/strict mode. Entry protocol 3 records SHA256 over canonical dtype, shape and contiguous latent bytes. The official builder uses atomic no-replace entry writes and publishes `cache_manifest.v1.json` only after all requested entries are revalidated. Strict configuration explicitly locks the manifest file SHA256. Each process validates and indexes that manifest once while its complete stat identity is stable; a stat change triggers a new locked-SHA check and parse, while every selected latent is still loaded and checked for content, dtype, shape, source and protocol. Stat is only a change detector. Entries also bind dataset namespace, camera, manifest/generation/label/file identities, source/target frames, actual frame/time delta, VAE config/weight identities, normalization, latent geometry, color scale and filter rules. Missing, legacy, forged, modified or incompatible entries fail with a rebuild instruction and are never overwritten or encoded online by strict mode.
- Checkpoint: V2 sidecars record type, architecture, query count, measured latent shape, Adapter shape/count/hash, complete target/cache protocol, VAE config hash and separately computed weight-content hash, normalization, color protocol/scale, filter rules and the strict cache manifest identity when applicable. Training restore compares these with the actual builder and Adapter; same-stage strict resume reconstructs the builder and validates the locked manifest before loading the Head. Deployment may omit the auxiliary branch without resolving the VAE. Legacy V1 sidecars remain readable and are never interpreted as V2. Cross-stage V1-to-V2 replacement requires an explicit type change and may specify a new cache; same-stage V2 resume requires complete weights and metadata.
- Optimizer semantics: V2 loss is the sum of FP32 per-sample latent element means divided by the global eligible-sample count. The same selector drives loss, metrics and distributed activity. An OF-only window with no eligible sample clears residual gradients and does not call optimizer/scheduler or advance successful-window state; a joint window still updates when AR/FM supervision is active. Existing AMP overflow result handling remains authoritative.
- Calibration: `scripts/calibrate_flow_color_scale.py` requires a version-1 JSON train frame allowlist bound to the exact Flow manifest SHA256 and dataset/camera identities. Protocol 4 also binds the default path to hashed dataset `meta/info.json` splits, `meta/stage05_episode_mapping.jsonl` and the dataset identity in `meta/stage05_merge.json`; Flow `source_episode_index` is mapped to the dataset episode before train membership is accepted, so validation/test rows are rejected without assuming episode IDs are equal. Sources without verifiable split metadata require explicit externally-trusted mode, recorded as not independently verified. Calibration uses a seeded, fixed-capacity uniform reservoir in bounded chunks and records the estimated quantile, authority/index evidence, included range, sample/observed counts, memory bound, seed, units, manifest/label identities, filter coverage and estimated truncation. The locked output is write-once; training never recalibrates per sample or batch.
- Calibration split authority must cover every dataset episode exactly once; incomplete or overlapping ranges are rejected before scale estimation. The Stage06 production adapter passes the verified auxiliary contract when registered, while the direct legacy-compatible adapter still uses the reader's mandatory actual-HDF5 SHA256 check without inventing a source mapping.
- Reader integrity/schema: before production Stage06, official cache generation or calibration consumes a Flow HDF5, actual content is verified against the manifest SHA256. The process-local verification record is bound to expected digest plus full stat identity; a change evicts old handles/row indices and revalidates, while stable samples avoid repeated full-file hashes. Current `stage06_flow_manifest_v2` / `stage06_flow_v2` files require finite float `[T]` `valid_fraction` in `[0,1]`, and each consumed row is compared with mask mean at absolute tolerance `1e-6`. Missing version markers are rejected. Only an explicit `stage06_flow_legacy_v1` contract permits the supported unversioned file and optional scalar.
- Fourth-review regression under `PYTHONNOUSERSITE=1 PYTHONPATH=.` and `/opt/data/private/lq/miniconda3/envs/ZR-0/bin/pytest`: V2 `46 passed, 1 skipped`; complete Flow/Stage05/Stage06/Slot reader group `78 passed`; optimizer/AMP/Query/checkpoint `95 passed`; review/config `48 passed`; Stage05 mixed sidecars `24 passed`. The real-Wan CUDA test remains default skipped; no GPU, cache, calibration, formal training or rollout was started.
- GPU test gate: real Wan CUDA pytest is disabled by default. It requires `ZR0_RUN_REAL_WAN_CUDA_TEST=1`, exactly one explicitly selected `CUDA_VISIBLE_DEVICES` entry, and a passing `utils.gpu_resource_gate.wait_for_gpus` check before VAE construction. This is a point-in-time resource check, not an exclusivity guarantee.
- Verification: focused V2 tests cover 0.95 filtering and exact MSE denominator, zero-supervision/joint optimizer behavior, two-rank Gloo normalization with a zero-supervision rank, official writer/manifest/strict-reader compatibility, latent/file/manifest tampering, existing-file conflicts, HDF5 content/replacement checks, process-local manifest indexing, cache source/label collisions, checkpoint/cache fingerprint tampering and VAE weight changes, current/explicit-legacy schema, authoritative split mapping, forged validation allowlists, BF16/FP16/FP32 Adapter inputs, scalar delta fallback, deployment omission and bounded calibration. Third-review CPU validation used three non-overlapping groups totaling 421 passed and one default-gated real-Wan CUDA skip; CLI/help, six-command launcher dry-run, compilation and diff checks passed. A real Tabletop row verified its actual HDF5 SHA256 before returning delta20/source1. CPU/fake-VAE evidence is not reported as real GPU/NCCL/ZeRO-2 validation. No formal V2 training, full cache build, full calibration or rollout was started.
- Environment: `/opt/data/private/lq/miniconda3/envs/ZR-0` has PyTorch 2.6.0+cu124, Diffusers 0.35.2 and the required runtime dependencies. `/opt/data/private/lq/models/Wan2.2-TI2V-5B-Diffusers` contains the complete Diffusers VAE. A prior real CPU probe on `[1,3,1,224,224]` returned `[1,48,1,14,14]`, and a real MolmoAct target completed latent MSE backward. Current PyTorch reports CUDA available, but post-fix real GPU migration/backward remains unverified because the four A800 devices were in active use.
...
## Completed LIBERO fine-tuning from Stage 3 H50 step 14000

- Date: 2026-09-12. Purpose: record the user-authorized downstream run from the current H50 pretraining checkpoint with LIBERO horizon 10 and the previous fine-tuning configuration. The H50 supervisor startup status above is historical: that training was subsequently stopped by the user before this LIBERO run.
- Files: `docs/experiments/libero_stage3_h50_step14000_20260912/experiment.md`, output `experiment.md`, this reference. Implementation source: direct reuse of `scripts/run_libero_wo_ecot_pt.sh`, `scripts/train_libero_finetune.py` and pinned 3eefb602 runtime; no training-code changes. Existing `difference_query_stage3` arm, source-path environment override and `ZR0_LIBERO_ACTION_HORIZON=10` select the run; other defaults/arms are unchanged.
- Input/output: VLM/Query/Expert from H50 Stage 3 step14000, source/runtime horizon 50/10, fresh AdamW/scheduler, LIBERO-only statistics and FM-only objective, full fine-tuning, 4×16×1=64, 8 epochs/34184 updates. Final output: `outputs/ckpts/ZR0-stage3-step14000-h50-LIBERO-action-only-dq32-h10-gbs64-seed42/step-34184`; full immutable recovery in `recovery_checkpoints/step-034184/latest-model-optimizer-lr`.
- Verification: finished 2026-09-12 17:25:44 +08:00 including W&B synchronization (run aok9gry8); initialization exact tensor checks passed, final 26-file inventory matches its receipt, loss/gradients finite in recorded metrics, no logged skips/OOM/fatal exception, no remaining ranks and four GPUs released. Final FM loss 0.00845337, minimum logged 0.00747561. No data re-audit, optimizer updates, resume replay or rollout evaluation during this completion inspection. The result documentation distinguishes observed completion from untested evaluation/resume equivalence.


## Resume original H50 Stage 3 from update 16000

- Date: 2026-09-12. Purpose: fulfill the explicit request to resume the original H50 pretraining at its complete 16000 checkpoint, with all training settings unchanged and cumulative target 150000. This supersedes the historical stopped H50 status; the completed LIBERO experiment is a separate run.
- Files: `scripts/run_stage3_h50.py`, `configs/three_stage_formal_stage3_h50_resume16000_20260912.json`, `tests/test_stage3_h50_resume_launcher.py`, `docs/experiments/stage3_h50_resume16000_20260912/experiment.md`, this reference.
- Implementation source: adaptation of the existing H50 `Experiment` launcher. Directly reuse pinned 3eefb602 `scripts/watch_three_stage_formal.py::{attempt_item,validate_checkpoint,RecoverySupervisor}` and existing `scripts/stage3_queue_checks.py::verify_startup`. No changes to the pinned trainer, model, datasets, optimizers or loss implementation; no external source.
- Switch/default: optional `resume_from_checkpoint`, `resume_step`, `resume_source_experiment` and `resume_source_launcher_commit` activate same-stage recovery. Without them, the original fresh Stage 3 plan remains unchanged. `--mode prepare` is CPU-only; explicit `--mode run` starts training. The resume branch rejects fresh-initialization fallback and any training hyperparameter drift from the sealed original H50 plan.
- Input/output and data flow: all five components, four-rank ZeRO-2/AdamW, scheduler, RNG, sampler cursor and exposure are restored from original `step-016000-attempt-000`. New logs/checkpoints are isolated under the original experiment's `resume_from16000_20260912` subdirectory. Source checkpoints/logs remain preserved. Online W&B retains the project/group and receives a new run identity to avoid overlap with the old unsaved 16001–16208 metrics.
- Compatibility: H50, Query32/Flow16, AR1 + Slot0.1 + Flow1 + FM5, 4×16×2=128, BF16/ZeRO-2/SDPA, LR1e-5/min1e-6, original cosine150000/warmup12000, saves2000, seed42 and fast sampler resume are unchanged. Frozen indices and independent dataset statistics are reused without payload re-audit. Only launch orchestration changes; its prior sealed bytes are verified against commit d17267551a79af3361f34f8b51f51b6b031a1ded, while all original runtime/training adapters and prepared artifact hashes must still match.
- Verification: CPU regression checks compare unchanged fresh commands and reject unintended resume horizon/LR/warmup/loss changes, using the preserved deployment artifacts. Complete source inventory/native recovery validation precedes launch. Four-rank exact saved-state checks precede the first resumed update; existing startup verification checks production updates 16001–16020. Actual results are recorded below after observation. No separate diagnostic optimizer updates, full data re-audit, rollout evaluation or claim of identical uninterrupted trajectory.
- Known limits: local command regressions require the preserved experiment artifacts. This startup verification does not independently replay a next-window forward comparison or cover an epoch boundary. Bounded automatic retries and UUID resource gates are retained. The supervisor continues only this Stage 3 to cumulative150000; no downstream run is queued.

- Actual resume result: six CPU regressions and sealed/native checkpoint checks passed. Four-rank training started 2026-09-12T19:47:29.827568+08:00; startup verified updates 16001–16020 and observed update 16030. All 845 source tensors and rank optimizer/master/scheduler/RNG/cursor states are exact; exposure is exact, with zero diagnostic updates and zero historical dataset reads. The initial restored LR is 9.98134647456885e-6. Updates 2–20 average 3.7830 s, peak allocated/reserved 39.1356/74.8027 GiB; gradients/losses finite, no observed skips or W&B failures. Evidence: new output `resume_startup_report.json` and attempt `startup_verified.json`; [online W&B](https://wandb.ai/jumbo3r-zhejiang-university/zr0-three-stage-formal/runs/stage3_h50_resume16000_20260912-stage3_joint-retry000). Training remains active toward 150000; next scheduled save 18000. Task changes staged, no commit.

## Fifth review calibration follow-up

- Date: 2026-09-12. Scope: two calibration-only fixes; model, latent loss,
  stage semantics, checkpoint and strict-cache behavior are unchanged.
- Camera/source authority: the default `scripts/calibrate_flow_color_scale.py`
  path now requires `meta/info.json`, `meta/modality.json` and the compact
  `meta/episodes` Parquet sidecar. Every manifest camera must be an image/video
  feature and a modality video key. Each mapping `source_data_uri` is resolved
  from the dataset root, with symlink resolution and root containment checks;
  it must be an in-root Parquet file. The sidecar binds `(chunk,file)` to the
  actual Stage05 target `episode_index` and per-episode length, while Parquet
  row-group statistics cover the complete file (a file may contain multiple
  episodes). The first check of each unique source file then reads only the
  `episode_index` column in fixed-size batches and compares exact per-episode
  counts; image/state/action columns are never decoded. The mapping preserves the original/source episode ID and its
  old-to-new target relation. Camera feature shape, modality digest, resolved
  source identity, file size and source/target episode identities are included
  in the calibration authority output. Missing, external, malformed,
  source-conflicting or unknown-camera inputs fail closed. Explicit
  `--externally-trusted-training-index` remains the only path that records
  `not_independently_verified`.
- Calibration block integrity: every 16-row block validates the original mask
  values (binary 0/1 before conversion), finite/in-range `valid_fraction`,
  mask-mean agreement and finite `[T,2,224,224]` flow before any train, delta,
  label-source or quality filter. Tail/short-delta and nonmatching source rows
  cannot hide malformed labels; qualified-frame selection and the reservoir/
  scale definition are unchanged.
- Fifth-review evidence: focused V2 tests pass `53 passed, 1 skipped` (the
  default real-Wan CUDA gate). Added fixtures cover camera/URI/episode identity,
  root failures, 17-row two-block ordering, nonbinary masks, invalid or
  mismatched fractions, and the one-qualified-frame scale. A read-only
  authority probe against MolmoAct Tabletop metadata passed in 2.91 seconds and
  confirmed `first_view`, target/source mapping and multi-episode Parquet files
  without modifying data. No real CUDA, NCCL, DeepSpeed, cache build,
  calibration job, formal training or rollout was run for this follow-up.
